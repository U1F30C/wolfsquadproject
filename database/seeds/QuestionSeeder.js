...
class QuestionSeeder {
  async run () {
    ...
    const question_2 = await Factory.model("App/Models/Question").create({
      description:"¿Has tenido dificultades porque consumes drogas o bebidass alcohólicas en la escuela?",
      
    });
    Database.table("questions").insert(question_2)
  
    const question_3 = await Factory.model("App/Models/Question").create({
      description:"¿Se aburren tus amigos en las fiestas donde no sirven bebidas alcohólicas?",
  ...
  const question_11 = await Factory.model("App/Models/Question").create({
    ...
    const question_14 = await Factory.model("App/Models/Question").create({
      description:"¿Se niegan tus padres o tutores a havlaarte cuando se enfadan contigo?",
      
    });
    Database.table("questions").insert(question_14)
  
    const question_15 = await Factory.model("App/Models/Question").create({
      description:"¿Actúas impulsivamente y sin pensar en las consecuencias que tendrán tus actos?",
    ...
    const question_17 = await Factory.model("App/Models/Question").create({
      description:"¿Te has hecho daño o les has hecho daño a otra parsona accidentalmente, estando bajo los efectos del alcohol o drogas?",
  ...
  const question_19 = await Factory.model("App/Models/Question").create({
      description:"¿Tienes amigos que causan daño o destrucción intencioanlmente?",
      
    });
    Database.table("questions").insert(question_19)
  
    const question_20 = await Factory.model("App/Models/Question").create({
      description:"La mayoria de las veces, ¿saben tus padres o tutores dónde estás y lo que estás haciendo?",
      
    });
    Database.table("questions").insert(question_20)
  
    const question_21 = await Factory.model("App/Models/Question").create({
      description:"¿Sueles perderte actividades o acontecimientos porque has gastado demasiado dinero en drogas o bebidas alcohólicas?",
      
    });
    Database.table("questions").insert(question_21)
  
  const question_22 = await Factory.model("App/Models/Question").create({
      description:"¿Participas en muchas actividades en compañía de tus padres o tutores?",
  ...
  const question_25 = await Factory.model("App/Models/Question").create({
      description:"¿Has sentido que eres adicto (a) al alcohol o a las drogas?",
  ...
  const question_27 = await Factory.model("App/Models/Question").create({
      description:"¿Has estado ausente o llegado tarde a tu trabajo o escuela con frecuencia?",
  ...
  const question_32 = await Factory.model("App/Models/Question").create({
      description:"¿Te prestan atención tus padres o tutores cuando les hablas?",
      
    });
    Database.table("questions").insert(question_32)
  
  const question_33 = await Factory.model("App/Models/Question").create({
      description:"¿Has comenzado a consumir mayores cantitades de drogas o alcohol para obtener el efecto que deseas?",
  ...
  const question_38 = await Factory.model("App/Models/Question").create({
      description:"¿A veces te vas de las fiestas porque en ellas no hay bebidas alcohólicas o drogas?",
      
    });
    Database.table("questions").insert(question_38)
  
  const question_39 = await Factory.model("App/Models/Question").create({
      description:"¿Saben tus padres o tutores cómo piensas o te sientes realmente?",
  ...
  const question_41 = await Factory.model("App/Models/Question").create({
      description:"¿Sientes un deseo constante de consumir bebidas alcohólicas o drogas?",
  ...
  const question_44 = await Factory.model("App/Models/Question").create({
      description:"¿Has tenido alguna vez un empleo con suueldo que haya durado por lo menos un mes?"
    });
    Database.table("questions").insert(question_44)
  
  const question_45 = await Factory.model("App/Models/Question").create({
      description:"¿Discutes frecuentemente con tus padres o tutores, levantando la voz y gritando?",
      
    });
    Database.table("questions").insert(question_45)
  
  const question_46 = await Factory.model("App/Models/Question").create({
      description:"¿Has tenido un accidente autmovilístico estando bajo los efectos del alcohol o drogas?",
  ...
  const question_48 = await Factory.model("App/Models/Question").create({
      description:"El mes pasado, ¿manejaste un automóvil estando borracho (a) o drogado (a)?",
  ...
  const question_51 = await Factory.model("App/Models/Question").create({
      description:"¿Has dejado un empleo sencillamente porque no te interesaban las consecuencias de dejarlo?"
    });
  ...
  const question_53 = await Factory.model("App/Models/Question").create({
      description:"¿Has pasado alguna noche fuera de tu casa sin que tus padres o tutores supieran donde estabas?"
    });
    Database.table("questions").insert(question_53)
  
  const question_54 = await Factory.model("App/Models/Question").create({
      description:"¿El uso del alcool o las drogas te produce cambios repentinos de humor, como pasar de estar contento (a) a estar triste, o viceversa?",
  ...
  const question_56 = await Factory.model("App/Models/Question").create({
      description:"¿Pierdes días de clase o llegas tarde a la escuela por haber consumido bebidas alcohólicas o drogas?",
      
    });
    Database.table("questions").insert(question_56)
  
  const question_57 = await Factory.model("App/Models/Question").create({
      description:"¿Te han dicho alguna vez tus familiares o amigos que debes reducir el uso de bebidas alcohólicas o drogas?",
      
    });
    Database.table("questions").insert(question_57)
  
  const question_58 = await Factory.model("App/Models/Question").create({
      description:"¿Discutes seriamente con tus amigos o familiares por el uso quue haces de bbidas alcohólicas o drogas?",
  ...
  const question_62 = await Factory.model("App/Models/Question").create({
      description:"Las bebidas alcohólicas o drogas, ¿te han inducido a hacer algo que normalmente no harías, como desobedecer alguna regla o ley, o a la hora de llegar a casa, o tener relaciones sexuales con alguien?",
      
    });
    Database.table("questions").insert(question_62)
  
  const question_63 = await Factory.model("App/Models/Question").create({
      description:"¿Sientes que a veces pierdes control de ti mismo(a) y terminas peleando?",
  ...
  const question_65 = await Factory.model("App/Models/Question").create({
      description:"¿Tienes dificultades en tuus relaciones con alguno de tus amigos debido a las bebidas alcohólicas o drogas que consumes?",
  ...
  const question_67 = await Factory.model("App/Models/Question").create({
      description:"¿Tienes amigos que han golpeado o amenazado a alguien sin razón?",
      
    });
    Database.table("questions").insert(question_67)
  
  const question_68 = await Factory.model("App/Models/Question").create({
      description:"¿Has sentido que no puedes controlar el deseo de consumir bebidas alcohólicas o drogas?",
  ...
  const question_70 = await Factory.model("App/Models/Question").create({
      description:"¿Tienen tus padres o tutores una idea relativamente buena de lo que te interesa?",
      
    });
    Database.table("questions").insert(question_70)
  
  const question_71 = await Factory.model("App/Models/Question").create({
      description:"¿Están tus padres o tutores de acuerdo en cuanto a la forma en que te deben educar?",
      
    });
    Database.table("questions").insert(question_71)
  
  const question_72 = await Factory.model("App/Models/Question").create({
      description:"¿Se te hace difícil hacer planes u organizar tus actividades?"
    });
    Database.table("questions").insert(question_72)
  
  const question_73 = await Factory.model("App/Models/Question").create({
      description:"¿Tus amigos faltan a la escuela sin autorización con mucha frecuencia?",
  ...
  const question_80 = await Factory.model("App/Models/Question").create({
      description:"¿Eres una persona nerviosa, de las que no pueden estar sentadas mucho tiempo?",
